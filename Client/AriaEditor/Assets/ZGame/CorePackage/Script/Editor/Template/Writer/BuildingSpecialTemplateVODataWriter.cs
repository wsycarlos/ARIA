// ------------------------------------------------------------------------------
//  <autogenerated>
//      This code was generated by a tool.
//      Mono Runtime Version: 2.0.50727.1433
// 
//      Changes to this file may cause incorrect behavior and will be lost if 
//      the code is regenerated.
//  </autogenerated>
// ------------------------------------------------------------------------------

namespace Game.Template.Editor
{
    using System.Collections.Generic;
    
    
    public class BuildingSpecialTemplateVODataWriter : ITemplaterWriter
    {
        
        public ByteArray GenerateByteArray(object data)
        {
            List<BuildingSpecialTemplateVO> list = data as List<BuildingSpecialTemplateVO>;
            ByteArray ba = new ByteArray();
            int length = list.Count;
            ba.WriteInt(length);
            for (int i = 0; (i < length); i = (i + 1))
            {
                ba.WriteInt(list[i].id);
                ba.WriteInt(list[i].buildingTempId);
                ba.WriteInt(list[i].level);
                ba.WriteInt(list[i].upgradeNeedLevel);
                ba.WriteInt(list[i].upgradeCdTime);
                ba.WriteInt(list[i].gold);
                ba.WriteInt(list[i].wood);
                ba.WriteInt(list[i].stone);
                ba.WriteInt(list[i].crystal);
                ba.WriteInt(list[i].sulfur);
                ba.WriteInt(list[i].diamond);
            }
            return ba;
        }
    }
}

...
internal class BundleTreeWin : EditorWindow
{
    ...
    private Dictionary<string, bool> m_PathFoldDict = new Dictionary<string, bool>();

    bool IsPathFold(string name)
    {
        if (!m_PathFoldDict.ContainsKey(name))
            m_PathFoldDict.Add(name, true);

        return m_PathFoldDict[name];
    }

    void SetPathFold(string name, bool isFold)
    {
        m_PathFoldDict[name] = isFold;
    }
}
